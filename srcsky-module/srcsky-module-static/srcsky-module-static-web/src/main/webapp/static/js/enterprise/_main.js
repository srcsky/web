...
(function (S) {
    ...
    // 获取 _main.js 所在目录
    S._script_path = (function (scripts,href) {
        S.each(scripts, function (script) {
            if (script.src && script.src.indexOf("/_main") != -1) {
                href = script.src.replace(/\\/g, '/').split("/_main")[0];
    ...
    S.mix(S, {
        ...
        data:function (name, value) {
            ...
            if (name === undefined) {
                return S.data_cache_stack;
            } else if (type === 'string') {
                if (value === undefined) {
                    return S.data_cache_stack[name];
                } else {
                    S.data_cache_stack[name] = value;
                }
            } else if (type === 'object') {
                for (var _property in name) {
                    S.data_cache_stack[_property] = name[_property];
                }
            }
        }, isEmpty:function ($obj, $obj2) {    // 非空验证
            if (undefined == $obj || null == $obj)return true;
            if ((typeof $obj) == "string")return S.trim($obj) == "" || S.trim($obj) == $obj2;
            if ((typeof $obj) == "number")return $obj <= 0 || $obj == $obj2;
            if ((typeof $obj) == "array")return $obj.length == 0;
            return false;
        }, getString:function ($src, $default) {  // 获取默认指
            return !S.isEmpty($src) ? $src : $default;
        }, getPath:function (path) { // 获取 跟目录
            return S._script_path + (path ? path : "");
        }
    });
    // Packages/资源加载路径 配置
    S.config({
...
//        base:"/static/js/enterprise/thirdparty/kissyui/1.3.0/",
        packages:{
            gallery:{
                debug:true,
                combine:false,
                base:"/static/js/enterprise/thirdparty/kissyui/"
            }, enterprise:{
                debug:true,
                combine:false,
                ignorePackageNameInUri:true,
                base:'/static/js/enterprise'
            }
        }
    });
})(KISSY);
