...
public class SWFUpload extends Sprite {
		...
		private var fileMinSizeLimit:Number;
		...
		private var SIZE_TOO_MIN:Number		= 2;
		...
		private var ERROR_CODE_FILE_EXCEEDS_SIZE_LIMIT_MIN:Number 	= -111;
		...
		public function SWFUpload() {
            ...
            this.buttonLoader.contentLoaderInfo.addEventListener(Event.COMPLETE, function(event:Event):void{

            });

			//this.buttonLoader.visible=false;
			...
			try {
				this.SetFileMinSizeLimit(String(root.loaderInfo.parameters.fileMinSizeLimit));
			} catch (ex:Object) {
				this.fileMinSizeLimit = 0;
			}
		...
		private function SetupExternalInterface():void {
			try {
				...
				ExternalInterface.addCallback("SetFileMinSizeLimit", this.SetFileMinSizeLimit);
		...
		* *************************************** */
		...
		private function Select_Handler(file_reference_list:Array):void {
			...
			if (queue_slots_remaining < file_reference_list.length) {
				this.Debug("Event: fileQueueError : Selected Files2 (" + file_reference_list.length + ") exceeds remaining Queue size (" + queue_slots_remaining + ").");
				...
				for (var i:Number = 0; i < file_reference_list.length; i++) {
					...
					if (is_valid_file_reference) {
						...
						if(size_result == this.SIZE_OK && is_valid_filetype) {
							...
							ExternalCall.FileQueueError(this.fileQueueError_Callback, this.ERROR_CODE_INVALID_FILETYPE, file_item.ToJavaScriptObject(), "文件类型错误");
						}
						else if (size_result == this.SIZE_TOO_MIN) {
							file_item.file_reference = null; 	// Cleanup the object
							this.queue_errors++;
							this.Debug("Event: fileQueueError : File exceeds size limit.");
							ExternalCall.FileQueueError(this.fileQueueError_Callback, this.ERROR_CODE_FILE_EXCEEDS_SIZE_LIMIT_MIN, file_item.ToJavaScriptObject(), "文件太小.");
						}
						else if (size_result == this.SIZE_TOO_BIG) {
							file_item.file_reference = null; 	// Cleanup the object
							this.queue_errors++;
							this.Debug("Event: fileQueueError : File exceeds size limit.");
							ExternalCall.FileQueueError(this.fileQueueError_Callback, this.ERROR_CODE_FILE_EXCEEDS_SIZE_LIMIT, file_item.ToJavaScriptObject(), "文件太大");
						}
						else if (size_result == this.SIZE_ZERO_BYTE) {
							file_item.file_reference = null; 	// Cleanup the object
							this.queue_errors++;
							this.Debug("Event: fileQueueError : File is zero bytes.");
							ExternalCall.FileQueueError(this.fileQueueError_Callback, this.ERROR_CODE_ZERO_BYTE_FILE, file_item.ToJavaScriptObject(), "文件不能为空");
						}
					} else {
						file_item.file_reference = null; 	// Cleanup the object
						this.queue_errors++;
		...
		****************************************************************** */
		...
		private function SetFileMinSizeLimit(size:String):void {
			var value:Number = 0;
			var unit:String = "kb";
			...
			this.fileMinSizeLimit = value * multiplier;
		}
		...
		*************************************************************** */
		private function StartUpload(file_id:String = ""):void {
			...
			if (this.successful_uploads >= this.fileUploadLimit && this.fileUploadLimit != 0) {
				...
				ExternalCall.UploadError(this.uploadError_Callback, this.ERROR_CODE_UPLOAD_LIMIT_EXCEEDED, null, "超过限定上传的文件数");
				this.current_file_item = null;
				return;
			}
		...
		*************************************************************** */
		...
		private function CheckFileSize(file_item:FileItem):Number {
			...
			}else if (this.fileMinSizeLimit != 0 && file_item.file_reference.size < this.fileMinSizeLimit) {
				return this.SIZE_TOO_MIN;
			} else {
				return this.SIZE_OK;
			}
		}
