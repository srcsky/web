...
KISSY.add('gallery/image-dd/1.0/index', function (S, Base, Template, Anim) {
    ...
    var ATTRS = {
        ...
        , initWidth:0, defaultMaxWidth:10000//图片放大到最大宽度
        , defaultMinWidth:50 //图片缩小到最小宽度
        , ieIframeMask:null, dragInfoX:[] //拖拽图片的坐标记录
        , dragInfoY:[], dragInfoTime:[] //拖拽发生的时间记录
        , autoSlideAnim:null //Anim obj

        , popupMask:null //全屏浮框容器
        , popupBd:null //弹出层，图片容器
        , popupOpacityBg:null, popupBox:null, popupHd:null
    };
    ...
    var DATAS = {
        POPUP_HD_TPL:'<div class="box-hd close-rt-wrap" ><a href="#" title="按退出键，也可以关闭哦" class="close-rt J_Close" id="J_CloseImageDD"></a></div>', POPUP_IMG:'<img title="鼠标滚轮可以放大图片" class="G_K" style="width:{{showWidth}}px;left:{{left}}px;top:{{top}}px;" src="{{imgSrc}}"  />', POPUP_TPL:'<div class="img-dd-mask">\
                    <div class="img-dd-opacity-bg"></div>\
                 </div>', POPUP_BOX_TPL:'<div class="img-dd-box"></div>', POPUP_IFRAME_TPL:'<iframe class="ie-popup-mask hidden"></iframe>'
    }
    ...
    var o = {
        ...
        , _bindEvent:function (ele, className) {
            ...
            E.on(ele, 'click', function (e) {
                ...
                self._showPopupImg(target.getAttribute('data-original-url') ? target.getAttribute('data-original-url') : target.getAttribute('_src_'), target.getAttribute('src'));
        ...
        , _showPopupImg:function (srcUrl, srcUrlThunmb) {
            ...
            self.ATTR('popupBd').innerHTML = Template(self.DATA('POPUP_IMG')).render({
                ...
                , imgAlt:'图片大图', showWidth:parseInt(clientWidth / 2, 10), left:parseInt(clientWidth / 4, 10) //将图片显示在正中间
                , top:0
            });
        ...
        , _bindPopupMousedown:function () {
            ...
            E.on(self.ATTR('popupBd'), 'mousedown', function (e) {
                ...
                self.ATTR('activeImgPos', {
                    left:getCurrentStyle(target, 'left'), top:getCurrentStyle(target, 'top')
                });
        ...
        , slide_straightLine:function () {
            ...
            var retBuffer = {
                a:100000, vx:0, vy:0, new_left:0, new_top:0, is_left:true, is_top:true
            };
            ...
            self.ATTR('autoSlideAnim', new Anim(self.ATTR('activeImg'), {
                left:retBuffer.addLeft, top:retBuffer.addTop
    ...
    function move(e) {
        ...
        var distance = {
            left:currentMouseCoo.x - self.ATTR('mousedownCoo').x, top:currentMouseCoo.y - self.ATTR('mousedownCoo').y
        };
    ...
    function wheelScroll(e) {
        ...
        //图片当前 宽高
        var activeImgSize = {
            width:getCurrentStyle(self.ATTR('activeImg'), 'width'), height:getCurrentStyle(self.ATTR('activeImg'), 'height')
        };
        ...
        //当前图片的偏移位置(相对body)
        var activeImgCoo = {
            'x':parseInt(self.ATTR('activeImg').style.left, 10), 'y':parseInt(self.ATTR('activeImg').style.top, 10)
        };

        //当前图片的偏移位置 (相对offsetParent)
        var activeImgOffset = {
            left:getCurrentStyle(self.ATTR('activeImg'), 'left'), top:getCurrentStyle(self.ATTR('activeImg'), 'top')
        }
        ...
        self.ATTR('anim', new Anim(self.ATTR('activeImg'), {
            left:parseInt(new_left * 1000, 10) / 1000 + 'px', top:parseInt(new_top * 1000, 10) / 1000 + 'px', width:parseInt(new_width, 10) + 'px'
        }, 0.1).run());
    }
    ...
    function getMouseCoo(e) {
        return {
            x:e.pageX || (e.clientX + (document.documentElement.scrollLeft || document.body.scrollLeft)), y:e.pageY || (e.clientY + (document.documentElement.scrollTop || document.body.scrollTop))
        }
    }
    ...
    function transToHump(s) {
        return s.replace(/-([a-z])/gi, function (i, j) {
    ...
    var _ddobj = new DDObj();

    return S.DDObj = {add:function (ele, className) {
        _ddobj.add(ele, className);
    }};
}, {
    requires:[
        './asdbase'
